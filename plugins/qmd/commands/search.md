---
description: BM25 keyword search (CLI only, MCP uses `query` tool with type "lex")
...
MCP equivalent: use the `query` tool with `{type: "lex", query: "..."}` for BM25 keyword search.
