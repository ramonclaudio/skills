...
If installed as a plugin, the `.mcp.json` handles configuration automatically. Don't also add it to `~/.claude/settings.json` -- duplicate servers are now auto-detected and skipped (v2.1.71).
...
The MCP server generates instructions at startup from actual index state. Injected into the LLM's system prompt via the MCP `initialize` response, giving immediate context without a tool call. LLMs automatically receive:
