...
WORKTREE=$(echo "$INPUT" | jq -r '.worktree.path // empty' 2>/dev/null)
AGENT_FIELDS=""
[ -n "$TOOL_USE_ID" ] && AGENT_FIELDS="${AGENT_FIELDS},\"tool_use_id\":\"${TOOL_USE_ID}\""
[ -n "$AGENT_ID" ] && AGENT_FIELDS="${AGENT_FIELDS},\"agent_id\":\"${AGENT_ID}\""
[ -n "$AGENT_TYPE" ] && AGENT_FIELDS="${AGENT_FIELDS},\"agent_type\":\"${AGENT_TYPE}\""
[ -n "$WORKTREE" ] && AGENT_FIELDS="${AGENT_FIELDS},\"worktree\":\"${WORKTREE}\""
...
case "$TOOL" in
  Bash)
    ...
    if [ "$IS_FAILURE" = "true" ]; then
      ...
      [ -z "$EXIT" ] && EXIT="error"
      echo "{\"ts\":\"${TS}\",\"type\":\"bash\",\"class\":\"${CLASS}\",\"cmd\":${CMD_JSON},\"exit\":\"${EXIT}\",\"error\":${ERROR_JSON}${AGENT_FIELDS}}" >> "$EVENTS_FILE"
    else
      ...
      echo "{\"ts\":\"${TS}\",\"type\":\"bash\",\"class\":\"${CLASS}\",\"cmd\":${CMD_JSON},\"exit\":\"${EXIT:-0}\"${AGENT_FIELDS}}" >> "$EVENTS_FILE"
  ...
  Write|Edit)
    ...
    if [ "$IS_FAILURE" = "true" ]; then
      ...
      ERROR=$(echo "$INPUT" | jq -r '(.error // "")[:200]' 2>/dev/null)
      ERROR_JSON=$(printf '%s' "$ERROR" | jq -Rs '.')
      echo "{\"ts\":\"${TS}\",\"type\":\"$(echo "$TOOL" | tr 'A-Z' 'a-z')_error\",\"file\":${FILE_JSON},\"error\":${ERROR_JSON}${AGENT_FIELDS}}" >> "$EVENTS_FILE"
    else
      echo "{\"ts\":\"${TS}\",\"type\":\"$(echo "$TOOL" | tr 'A-Z' 'a-z')\",\"file\":${FILE_JSON}${AGENT_FIELDS}}" >> "$EVENTS_FILE"
    fi
    ;;
esac
